...
/**
 * Class used to start the river.River JavaSpace service and allow a client
...
public class River extends Thread {
    ...
    public void run() {
        if (!new File(RiverConfiguration.POLICY).exists()) {
            System.err.println("Unable to start the river.River services: no \"" + RiverConfiguration.POLICY + "\" file");
        } else {
            try {
                ...
                System.out.println("Error stream assigned to \"logs/river.err\"");
                File dir = new File("logs");
                dir.mkdirs();
                System.setErr(new PrintStream("logs/river.err"));

                RiverConfiguration config = new RiverConfiguration(HOST, PORT);
                ServiceStarter.main(config);
                System.out.println("river.River services successfully started at " + HOST + ":" + PORT);
            } catch (Exception ex) {
                System.err.println("Unable to start the river.River services");
                ex.printStackTrace();
            }
        }
    }
