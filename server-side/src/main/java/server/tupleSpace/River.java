...
/**
 * Class used to start the server.tupleSpace.River JavaSpace service and allow a client
...
public class River extends Thread {
    ...
    private static void usage() {
        System.out.println("Usage: java server.tupleSpace.River [options]");
    ...
    public void run() {
        if (!new File(RiverConfiguration.POLICY).exists()) {
            System.err.println("Unable to start the server.tupleSpace.River services: no \"" + RiverConfiguration.POLICY + "\" file");
        } else {
            try {
                ...
                System.out.println("server.tupleSpace.River services successfully started at " + HOST + ":" + PORT);
            } catch (Exception ex) {
                System.err.println("Unable to start the server.tupleSpace.River services");
                ex.printStackTrace();
            }
        }
    }
